...
var THIS_SERVER_UDP_PORT_1 = 5555; //to receice  status (station state info)
var THIS_SERVER_UDP_PORT_2 = 5556; //to associate station ID with  its role task/activity
...
var udpdserver1 = dgram.createSocket('udp4'); //to receice station state info
var udpdserver2 = dgram.createSocket('udp4'); //associate station id and its role  activity (queue, assembly, cabling, etc)
...
udpdserver1.on('listening', function () {
    ...
    console.log('UDP Server station state  listening on ' + address.address + ":" + address.port);
});

udpdserver2.on('listening', function () {
    var address = udpdserver2.address();
    console.log('UDP Server station ID - task mapping  listening on ' + address.address + ":" + address.port);
});
...
udpdserver2.on('message', function (message, remote) {
    ...
    if(ioSocket){
        //console.log("inside socket");
        //console.log(msg);
        ioSocket.emit('station_mapping_info', msg); //
     };
});

...
#gAgvLocation = "NONE" #*1) this is for the smal setup; station1 original role queue, the tags below in Location enurataion class
                                                       #station2 original role is assembly, the tags below in Location enurataion class
                                                       #station3 original role is cabling, the tags below in Location enurataion class
                                                       #station4 original role is programming, the tags below in Location enurataion class
                                                       #station5 original role is inspection, the tags below in Location enurataion class
...
gStationRoleStaticDict = { #static map for local server back translation
    "QUEUE":"station1",
    "ASSEMBLY":"station2",
    "CABLING":"station3",
...
gStationRoleDict = { #these values are the original ones....to be changed dynamically for small setup(locally station ID changes)
  "QUEUE":"", #these values are the original ones....to be changed dynamically
  "ASSEMBLY":"",
  "CABLING":"",
  "PROGRAMMING":"",
  "INSPECTION":"",
...
def getAgvLocation(nfc_tag):
    ...
    global gStationRoleStaticDict
    location = '' 
    for data in Location:
        if data.value == nfc_tag:
            location = str(data.name)
            #print(location)
    for key in gStationRoleDict:
        #print(key)
        #print(gStationRoleDict[key])
        #print(gStationRoleStaticDict[key])
        if gStationRoleDict[key] == location:
           #print(gStationRoleDict[key])
           #print(gStationRoleStaticDict[key])
           return gStationRoleStaticDict[key]
    return ''
...
def on_message(data):
    ...
    role = msg[1]
    for key in gStationRoleDict: #TODO:  in order to support more than one station initialize only when role is NONE
       gStationRoleDict[key] = ""
    gStationRoleDict[role] = addressedStation
    gStationRoleDict["NONE"] = "UNDEFINED"
